-- SplitByMats_selected 0.0.1
-- (c) 2024 Mike Oxmaul
-- https://github.com/oxmaulmike2581
-- Licensed under MIT License - https://opensource.org/license/mit/

macroScript SplitByMats_selected
category:"Scripts by Mike"
buttonText:"SplitSelByMats"
--toolTip:"Split selected object by its materials"
silentErrors:true
(
	...
	on execute do
	(
		local sceneObjs = GetCurrentSelection()
		
		-- Check for empty scene
		if (sceneObjs.count < 1) then
		(
			messageBox "Select a object first!" title:"Error"
		)
		else
		(
